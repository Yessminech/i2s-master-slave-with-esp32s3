...
#define I2S_DATA 48
#define DMABufferLength (2046) //todo
...
ReadStatus fillSample(EventGroupHandle_t flags, int16_t *buffer, size_t totalSize, size_t *cursor)
{
    *cursor = 0;
    ...
    {
        size_t bytesToRead = (totalSize - *cursor > DMABufferLength) ? DMABufferLength : totalSize - *cursor;
        size_t bytesRead = 0;
        esp_err_t ret = i2s_channel_read(rx_handle, buffer + *cursor, 2 * bytesToRead, &bytesRead, 1000);
